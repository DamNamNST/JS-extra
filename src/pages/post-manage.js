...
const ManageNewsPost = {
  async render() {
    ...
    return /*html*/`
            ...
            <a href="/admin/post/add" class="btn btn-primary bg-blue-700 min-w-full text-white text-center px-4 py-2 mb-8 inline-block">Add product</a> 
                <table class="table table-bordered table-auto mx-auto">
                  <thead>
                    <tr>
                      <th>#</th>
                      <th>Tên sản phẩm</th>
                  ...
                  <tbody>
                    ${data.map((item, index) => /*html*/ `
                      <tr>
                        ...
                        <td>${item.content}</td>
                        <td>
                          <button class="btn btn-remove bg-red-600 text-white px-4 py-2" data-id=${item.id}>Remove</button>
                          <a href="/admin/post/${item.id}/update" class="btn bg-green-600 text-white px-4 py-2">Update</a>
                        </td>
                      </tr>
                      `).join('')}                        
                    </tbody>
